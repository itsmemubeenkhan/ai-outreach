<x-app-layout>
                ...
                <aside class="space-y-5">
                    <section class="rounded-2xl border bg-white p-6 shadow-sm">
                        ...
                        <div id="ai-result" class="hidden">
                            ...
                            <h3 class="mt-5 text-xs font-black uppercase tracking-wide text-slate-500">10 killer call starters</h3><ol id="ai-starters" class="mt-2 list-decimal space-y-2 pl-5 text-sm text-slate-700"></ol>
    ...
    @if($session?->currentLead)
        <script>
            ...
            const loadInsight=async()=>{try{const response=await fetch(@json(route('leads.sales-insight',$lead)),{headers:{'Accept':'application/json'}});const data=await response.json();if(!response.ok)throw new Error(data.message||'Analysis failed.');document.getElementById('ai-loading').classList.add('hidden');document.getElementById('ai-result').classList.remove('hidden');document.getElementById('ai-best-offer').textContent=data.analysis.best_offer||'Sales recommendation';document.getElementById('ai-summary').textContent=data.analysis.summary||'';document.getElementById('ai-reasons').innerHTML=listItems(data.analysis.reasons);document.getElementById('ai-opening').textContent=data.analysis.opening_pitch||'';document.getElementById('ai-starters').innerHTML=listItems(data.analysis.pitch_starters);document.getElementById('ai-questions').innerHTML=listItems(data.analysis.discovery_questions);const site=data.website;const missing=site.status==='missing';document.getElementById('website-intelligence').innerHTML=`<div class="${missing?'rounded-xl border border-amber-200 bg-amber-50 p-5':''}"><h3 class="text-xl font-black ${missing?'text-amber-950':'text-slate-950'}">${escapeHtml(site.title||'Website content')}</h3><p class="mt-2 ${missing?'text-amber-800':'text-slate-600'}">${escapeHtml(site.description||'No meta description found.')}</p></div>${missing?'':`<h4 class="mt-5 text-xs font-black uppercase tracking-wide text-slate-400">Pages and service signals</h4><ul class="mt-2 grid gap-2 md:grid-cols-2">${listItems(site.headings)}</ul>`}<h4 class="mt-5 text-xs font-black uppercase tracking-wide text-slate-400">AI website findings</h4><ul class="mt-2 space-y-2">${listItems(data.analysis.website_findings)}</ul>`;}catch(error){document.getElementById('ai-loading').classList.add('hidden');document.getElementById('ai-error').textContent=error.message;document.getElementById('ai-error').classList.remove('hidden');document.getElementById('website-intelligence').textContent=error.message;}};
            loadInsight();
