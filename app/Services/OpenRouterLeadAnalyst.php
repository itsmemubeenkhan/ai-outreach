...
class OpenRouterLeadAnalyst
{
    public function analyze(Lead $lead, array $website): array
    {
        ...
        $prompt = "Analyze this sales lead and its website content. Recommend only services justified by evidence. Never invent facts. If WEBSITE status is missing, explicitly state that no website is recorded and prioritize an appropriate website offer without pretending to have audited one. Create exactly 10 distinct, punchy, personalized call-opening starters a human salesperson can say. Return ONLY valid JSON with keys summary (string), best_offer (string), reasons (array of strings), website_findings (array of strings), opening_pitch (string), pitch_starters (array of exactly 10 strings), discovery_questions (array of strings), cautions (array of strings).\n\nLEAD:\n".json_encode($leadData)."\n\nWEBSITE:\n".json_encode($website);
