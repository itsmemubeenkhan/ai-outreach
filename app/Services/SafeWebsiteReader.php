...
class SafeWebsiteReader
{
    ...
    private function fetch(string $url): array
    {
        ...
        $port = ($parts['scheme'] === 'https') ? 443 : 80;
            $response = Http::timeout(10)->connectTimeout(4)->withUserAgent('AI-Outreach-CRM/1.0')->withOptions(['allow_redirects' => false, 'curl' => [CURLOPT_RESOLVE => ["{$host}:{$port}:{$ips[0]}"]]])->get($url);
            if (! in_array($response->status(), [301, 302, 303, 307, 308], true)) {
                if (! $response->successful()) throw new RuntimeException('Website returned HTTP '.$response->status().'.');
                return [$url, $response];
            }
            $location = $response->header('Location');
            if (! $location) throw new RuntimeException('Website returned an invalid redirect.');
            if (str_starts_with($location, '/')) $location = $parts['scheme'].'://'.$host.$location;
            $url = $location;
        }
